...
export const metadata: Metadata = {
  title: 'Profissionais de Educação Física — Encontre os melhores | ClinicaFácil',
  description: 'Saúde, condicionamento e performance. Personal trainers, preparadores físicos e profissionais de educação física.',
  alternates: { canonical: 'https://clinicafacil.com.br/profissionais/educacao-fisica' },
};

const ABORDAGENS = ["Personal Trainer","Preparação Física","Treinamento Funcional","Emagrecimento","Hipertrofia/Musculação","CrossFit","Yoga/Pilates","Treinamento para Idosos","Reabilitação Cardíaca"];

export default async function Page() {
  let data: any = { profissionais: [], total: 0 };
  try {
  data = await api.professionals.search({ especialidade: 'Educação Física', pagina: '1', tamanhoPagina: '20', ordem: 'relevancia' });
} catch {
  data = {
    total: 3,
    profissionais: [
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <header className="bg-white border-b sticky top-0 z-50 shadow-sm"><div className="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between"><Link href="/" className="text-2xl font-bold text-primary">ClinicaFácil</Link><Link href="/busca?especialidade=Educa%C3%A7%C3%A3o%20F%C3%ADsica" className="text-sm font-medium px-4 py-2 rounded-lg bg-primary text-white hover:bg-primary/90">Buscar mais</Link></div></header>
      <section className="bg-gradient-to-br from-red-50 to-white py-20">
        <div className="max-w-4xl mx-auto px-4 text-center">
          <h1 className="text-4xl md:text-5xl font-bold mb-4">Profissionais de Educação Física verificados</h1>
          <p className="text-lg text-muted-foreground max-w-2xl mx-auto">Saúde, condicionamento e performance. Personal trainers, preparadores físicos e profissionais de educação física.</p>
        </div>
      </section>
