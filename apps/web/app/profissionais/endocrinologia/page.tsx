...
export const metadata: Metadata = {
  title: 'Endocrinologistas — Encontre os melhores | ClinicaFácil',
  description: 'Hormônios e metabolismo. Endocrinologistas com CRM validado. Diabetes, tireoide, obesidade.',
  alternates: { canonical: 'https://clinicafacil.com.br/profissionais/endocrinologia' },
};

const ABORDAGENS = ["Diabetes","Tireoide","Obesidade/Metabologia","Reposição Hormonal","Osteoporose"," Endocrinologia Infantil","Síndrome Metabólica","Nutrologia","Doenças da Hipófise"];

export default async function Page() {
  let data: any = { profissionais: [], total: 0 };
  try {
  data = await api.professionals.search({ especialidade: 'Endocrinologia', pagina: '1', tamanhoPagina: '20', ordem: 'relevancia' });
} catch {
  data = {
    total: 3,
    profissionais: [
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <header className="bg-white border-b sticky top-0 z-50 shadow-sm"><div className="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between"><Link href="/" className="text-2xl font-bold text-primary">ClinicaFácil</Link><Link href="/busca?especialidade=Endocrinologia" className="text-sm font-medium px-4 py-2 rounded-lg bg-primary text-white hover:bg-primary/90">Buscar mais</Link></div></header>
      <section className="bg-gradient-to-br from-emerald-50 to-white py-20">
        <div className="max-w-4xl mx-auto px-4 text-center">
          <h1 className="text-4xl md:text-5xl font-bold mb-4">Endocrinologistas verificados</h1>
          <p className="text-lg text-muted-foreground max-w-2xl mx-auto">Hormônios e metabolismo. Endocrinologistas com CRM validado. Diabetes, tireoide, obesidade.</p>
        </div>
      </section>
